...
// Default options
const defaults = {
	url     : 'https://api-etc.ethermine.org',
	version : 0,
	timeout : 5000,
};
...
const rawRequest = async (url, headers, data, timeout) => {
	...
	headers['User-Agent'] = 'Etcmine Javascript API Client';
	...
	if(response.error && response.error.length) {
		...
		if(!error.length) {
			throw new Error("Etcmine API returned an unknown error");
		}
...
/**
 * EtcminelClient connects to the Etcmine.org API
